...
class DataInterCatering(win32serviceutil.ServiceFramework):
    ...
    _svc_name_ = "CSPDataInter"
    _svc_display_name_ = "Data interface CSP"
    _svc_description_ = "This is a data interface of CSP System."
    ...
    def SvcDoRun(self):
        ...
        try:
            ...
            sException = []
            try:
                schedule.every(self.sett.jdBillInterval).minutes.do(self.inter.trans_jos_homefw_task)
            except Exception as e:
                sError = str(e)
                self.sett.logger.error(sError)
            while self.sett.run:
